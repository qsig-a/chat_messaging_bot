...
- `.github/workflows/publish-image.yml` — builds and pushes
  ...
  unedited — tagged `vX.Y.Z`, `vX.Y`, and `latest`, matching the git tag. The `v` in
  `metadata-action`'s `pattern=v{{version}}` is load-bearing: a bare `{{version}}`
  normalises `v1.2.0` to `1.2.0` and the two tag spellings drift apart. Images up to
  `0.2.0` predate this and keep the bare form. Platforms come from the job-level
